...
from analyzer.analyze_utils.models import EventMessage
from analyzer.db.models import Message


class AMQPPublisher(Thread):
    ...
    def __init__(self, host: str, user: str, password: str, exchange_name: str, routing_key: str, queue: str,
        ...
        self._connection_params = pika.ConnectionParameters(host=host, port=5672, connection_attempts=20,
                                                            credentials=pika.PlainCredentials(username=user,
                                                                                              password=password))
        self._stomp_connection = stomp.Connection([('localhost', 61613)])
        self._stomp_connection.connect(user, password, wait=True)
    ...
    def on_connection_open_error(self, _unused_connection, err):
        ...
        """
        self._logger.error('Connection open failed, reopening in 5 seconds: %s', err)
    ...
    def on_connection_closed(self, _unused_connection, reason):
        ...
        if self._stopping.is_set():
            ...
            self._logger.warning('Connection closed, reopening in 5 seconds: %s',
                                 reason)
            self._connection.ioloop.call_later(5, self._connection.ioloop.stop)
    ...
    def on_queue_declareok(self, _unused_frame):
        ...
        """
        self._logger.info('Binding %s to %s with %s', self._exchange, self._queue,
                          self._routing_key)
    ...
    def on_delivery_confirmation(self, method_frame):
        ...
        self._logger.info('Received %s for delivery tag: %i', confirmation_type,
